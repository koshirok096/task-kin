...
export default function CreateInvitationModal({ open, onClose, groupInfo }) {
  ...
  const fetchGroupInfo = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_PROD_API_URL}/group/${groupInfo}`, {
        headers: {
  ...
  const createInvitation = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_PROD_API_URL}/invite/${groupInfo}`, {
        email: email
      }, {
        headers: {
          Authorization: token // Here
